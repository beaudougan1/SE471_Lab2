public class Price_CriteriaGT implements FilteringProviderIF{
    ...
    public Product[] runFiltering(Product[] products, String filter) {
        ...
        //create count variable to store current position in array
        int count = 0;
        //convert the filter given to a double value
        double filterPrice = Double.parseDouble(filter);
        //loop through the array of products
        for (int i = 0; i < products.length; i++) {
            //check if the product price is greater than or equal to the price to filter
